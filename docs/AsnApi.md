...
[**close_asn**](AsnApi.md#close_asn) | **POST** /beta/asn/closeAsn | Run the ClosePurchProcess process.
...
# **close_asn**
> list[ProcessOutputAPIModel] close_asn(body)
...
# create an instance of the API class
api_instance = Infoplus.AsnApi(Infoplus.ApiClient(configuration))
body = Infoplus.ClosePurchProcessInputAPIModel() # ClosePurchProcessInputAPIModel | Input data for ClosePurchProcess process.

try:
    # Run the ClosePurchProcess process.
    api_response = api_instance.close_asn(body)
    pprint(api_response)
except ApiException as e:
    print("Exception when calling AsnApi->close_asn: %s\n" % e)
```
