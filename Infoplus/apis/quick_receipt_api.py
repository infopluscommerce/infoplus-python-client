...
class QuickReceiptApi(object):
    ...
    def execute_quick_receipt(self, body, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.execute_quick_receipt(body, callback=callback_function)
        ...
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `execute_quick_receipt`")

        resource_path = '/beta/quickReceipt/executeQuickReceipt'.replace('{format}', 'json')
        path_params = {}
